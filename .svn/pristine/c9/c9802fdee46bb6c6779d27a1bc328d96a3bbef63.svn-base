...
<style>
    ...
    <form id="myForm" name="myForm" role="user" ng-submit="saveBtn()" novalidate>
        <h4 class="popup_title">编辑用户</h4>
            <div class="add_account clearfix mb10">
                ...
                <div class="red_note" style="color:red" ng-show="myForm.uno.$dirty && myForm.uno.$invalid">
                    <span ng-show="myForm.uno.$error.required">员工号必填</span>
                </div>
            </div>
            <div class="add_account clearfix mb10">
                <label>域用户<span style="color:red">*</span>：</label>
                ...
                <div class="red_note" style="color:red" ng-show="myForm.username.$dirty && myForm.username.$invalid">
                    <span ng-show="myForm.username.$error.required">域用户必填</span>
                </div>
            </div>
            <div class="add_account clearfix mb10">
                <label>Mac地址：</label>
        ...
        <div class="center_btn_a clearfix">
            <input class="button has_m_btn input_btn" type="submit" ng-disabled="myForm.username.$dirty &&myForm.username.$invalid||myForm.nickname.$dirty &&myForm.nickname.$invalid" value="保存"/>
